...
describe("currencyFormat & converter utils testing", () => {
  ...
  it("return only decimal number or mask string", () => {
    expect(getOnlyDecimal({ value: 5003.324, minLength: 3, zeroMask: '--' })).toBe('324')
    expect(getOnlyDecimal({ value: 5003, minLength: 3, zeroMask: '--' })).toBe('--')
    expect(getOnlyDecimal({ value: 5003.30003123, minLength: 6, noneZeroCnt: 4, zeroMask: '--' })).toBe('3000312')
  })
  ...
  it("returns the formatted currency", () => {
    expect(currencyFormat({ value: 520032.1234, prefix: '₩' })).toBe('₩520,032.12')
    expect(currencyFormat({ value: 10000.1234, prefix: '₩', includeSeparator: false })).toBe('₩10000.12')
  ...
  it("return converted units", () => {
    expect(convertUnits(-123.1234, 'krw')).toBe(`-${CURRENCIES.krw.symbol}123.123`)
    expect(convertUnits(123123123.1234, 'krw')).toBe(`${CURRENCIES.krw.symbol}1.23억`)
    expect(convertUnits(123123123123123.1234, 'krw')).toBe(`${CURRENCIES.krw.symbol}123.12조`)
    expect(convertUnits(12312312312312312.1234, 'usd')).toBe(`${CURRENCIES.usd.symbol}12.31Q`)
    expect(convertUnits(123123123123.1234, 'usd')).toBe(`${CURRENCIES.usd.symbol}123.12B`)
    expect(convertUnits(123123123.1234, 'usd')).toBe(`${CURRENCIES.usd.symbol}123.12M`)
    expect(convertUnits(1231.1234, 'usd')).toBe(`${CURRENCIES.usd.symbol}1,231.123`)

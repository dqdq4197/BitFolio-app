...
import krwFormat from './krw-format'
...
export default function convertUnits(
  ...
  const numToString = isNegative
    ? Math.floor(num).toString().substring(1)
    : Math.floor(num).toString()
  const numLen = numToString.length
  const currencyMark = CURRENCIES[currency].symbol
  let result = ''
  ...
  if (currency === 'krw') {
    ...
    switch (numToString.length) {
      ...
      case 7:
      case 8:
        result = `${numToString.substr(0, numLen - 4)}.${numToString.substr(
          numLen - 4,
          2
        )}만`
        break
      ...
      case 11:
      case 12:
        result = `${numToString.substr(0, numLen - 8)}.${numToString.substr(
          numLen - 8,
          2
        )}억`
        break
      ...
      case 19:
      case 20:
        result = `${numToString.substr(0, numLen - 16)}.${numToString.substr(
          numLen - 16,
          2
        )}경`
        break

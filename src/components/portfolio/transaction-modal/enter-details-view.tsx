...
import useHistorySnapshot from '@/hooks/data/use-history-snapshot'
...
import NumericPad from '@/components/common/numeric-pad'
import SetQuantityView from './set-quantity-view'
import SetDateView from './set-date-view'
import SetPricePerCoinView from './set-price-per-coin-view'
import SetFeeView from './set-fee-view'
import SetNotesView from './set-notes-view'
import { FormData, NumericData, SettingsType, FocusedView } from './form-modal'
            ...
            [focusedView]: {
              [currency]:
                prev[dummyKey]![currency].toString().length === 1
                  ? '0'
                  : prev[dummyKey]![currency].toString().slice(
                      0,
                      prev[dummyKey]![currency].toString().length - 1
                    ),
            },
    ...
    <Container $height={VIEW_HEIGHT}>
      ...
      >
        <SetQuantityView
          ...
          pricePerCoin={
            formData.pricePerCoin
              ? formData.pricePerCoin[currency].toString()
              : '0'
          }
        ...
        <SetPricePerCoinView
          height={VIEW_HEIGHT - NUMERIC_PAD_HEIGHT}
          pricePerCoin={
            dummyFormData.pricePerCoin
              ? dummyFormData.pricePerCoin[currency].toString()
              : '0'
          }
      ...
      >
        <NumericPad
          ...
          onNumericKeyPress={
            focusedView === 'pricePerCoin'
              ? handlePricePercoinNumericKeyPress
              : handleNumericKeyPress
          }
          onBackspacePress={
            focusedView === 'pricePerCoin'
              ? handlePricePerCoinBackspacePress

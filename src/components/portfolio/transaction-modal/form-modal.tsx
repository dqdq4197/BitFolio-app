...
import ScrollCloseModal from '@/components/common/scroll-close-modal'
import Text from '@/components/common/text'
import Image from '@/components/common/image'
import AsyncButton from '@/components/common/async-button'
import EnterDetailsView from './enter-details-view'
import SettingsSelectionBar from './settings-selection-bar'
...
const SETTINGS: SettingsType[] = [
  {
    ...
    key: 'quantity',
    icon: (color: string) => (
      <FontAwesome5 name="coins" size={14} color={color} />
    ),
  },
  {
    name: 'Date',
    key: 'date',
    icon: (color: string) => (
      <MaterialIcons name="date-range" size={14} color={color} />
    ),
  },
  {
    name: 'Price per coin',
    key: 'pricePerCoin',
    icon: (color: string) => (
      <FontAwesome name="dollar" size={14} color={color} />
    ),
  },
  {
    name: 'Fee',
...
const FormModalLayout = ({
  ...
  const currencyConverter = (): FormData<SubmitNumericData> => {
    ...
    pricePerCoin =
      typeof pricePerCoin === 'string' ? parseFloat(pricePerCoin) : pricePerCoin
          ...
          {activePage === 2 && (
            ...
            >
              <BackButtonWrap
                ...
                hitSlop={{
                  top: 5,
                  left: parseInt(theme.content.spacing, 10),
                  bottom: 5,
                }}
              >
                <MaterialIcons
        ...
        <AsyncButton
          text={
            activePage === 1
              ? t(`common.next`)
              : transactionId
              ? t(`common.edit transaction`)
              : t(`portfolio.add transaction`)
          }
          height={FOOTER_HEIGHT - 30}
          isLoading={isLoading}
          isDisabled={
            (formData.quantity === '0' && activePage === 2) || isLoading
          }
          onPress={
            activePage === 1 ? handleNextPress : handleAddTransactionPress
          }
          borderPosition={['top']}
          fontML
          hasNotch

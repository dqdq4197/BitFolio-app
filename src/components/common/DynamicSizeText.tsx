...
type SizeType = 'font_xl' | 'font_x' | 'font_l' | 'font_ml' | 'font_m' | 'font_s' | 'font_xs'
...
const DynamicSizeText = ({ children, defaultSize = 'font_m', ...props }: TextProps) => {

  const [sizeArr, setSizeArr] = useState<sizeArrType>([]);
  
  useEffect(() => {
    const sizes: sizeArrType = ['font_xl', 'font_x', 'font_l', 'font_ml', 'font_m', 'font_s', 'font_xs'];
    
    const index = sizes.findIndex(size => size === defaultSize);
    let newSizes = sizes.slice(index);

    ...
    if (coinsData) {
      ...
      Object.keys(coinStats).map(key => {
        ...
        coinStats[key].holding_costs =
          coinStats[key].holding_quantity * coinStats[key].price;

        const orderPL =
          coinStats[key].order_quantity * coinStats[key].price -
          coinStats[key].order_income_costs;
        const transferPL =
          coinStats[key].price * coinStats[key].transfer_quantity;

        coinStats[key].all_time_pl = orderPL + transferPL;
        coinStats[key].all_time_pl_percentage =
          (coinStats[key].all_time_pl / coinStats[key].total_purchase_cost) *
          100;

        const income_quantity_after_24h =
          coinStats[key].holding_quantity -
          coinStats[key].income_quantity_within_24h;
        const pl_after_24h = coinStats[key].price_change_24h
          ? coinStats[key].price_change_24h! * income_quantity_after_24h
          : 0;

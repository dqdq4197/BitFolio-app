...
export type ContentsType = ParagraphType | DelimiterType | EmbedType | ListType;
...
type Action =
  ...
  | { type: 'MERGE_PREVIOUS_LINE_WITH_CURRENT_LINE'; context: ParagraphType; focusIndex: number }
...
type HandlersType = {
  ...
  mergePreviousLineWithCurrentLine: (context: ParagraphType, focusIndex: number) => void,
...
export function MdEditorProvider({ children }:ProviderProps) {
  ...
  const handlers = {
    ...
    mergePreviousLineWithCurrentLine: (context: ParagraphType, focusIndex: number) => {
      dispatch({
